...
// Create Shortcode
// Modify fleet_shortcode function
function fleet_shortcode($atts) {
    ...
    if ($fleet_query->have_posts()) {
        echo '<div class="fleet-container">';  // Add this line
        ...
        if (!empty($loa)) {
            ...
            $show_plus_fleet = false; // Check if any PLUS+ Fleet checkbox is selected
            for ($i = 1; $i <= 6; $i++) {
                if (get_post_meta(get_the_ID(), "location$i", true) == 'on') {
                    $locations = [
                        1 => 'Ocean City',
                        2 => 'Somers Point',
                        3 => 'Avalon',
                    ...
                    $plus_fleet_checked = get_post_meta(get_the_ID(), "plus_fleet_location$i", true) == 'on' ? true : false;

                    if ($plus_fleet_checked) {
                        $show_plus_fleet = true; // Set to true if any PLUS+ Fleet checkbox is selected
